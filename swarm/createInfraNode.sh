...
if [[ $? -ne 0 ]] ; then
  ...
  if [ $AWS_ACCESS_KEY ]; then
    ...
    docker-machine -D create --driver amazonec2 --amazonec2-access-key $AWS_ACCESS_KEY --amazonec2-secret-key $AWS_SECRET_KEY --amazonec2-vpc-id $AWS_VPC_ID --amazonec2-region $AWS_REGION --amazonec2-zone $AWS_ZONE infra
  else
